...
# Abre os dados sobre as frases.
phrases_file = open('phrases.json', 'r')
...
if tweet:
    phraseid = int(dolar_real.replace('.', ''))
    dolar_real = dolar_real.replace('.', ',')
    name = phrases[phraseid - 1]

    imagepath = 'phrases_img/{}.png'.format(phraseid)
    emoji = ''
    if not first_tweet:
        ...
        status_template = "quanto tá o dólar? R${}\n\n #{} - {}"

    # realiza o tweet e salva a navoa cotação
    status = status_template.format(dolar_real, emoji, phraseid, name)
